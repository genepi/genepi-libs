...
public class LocalImporterSftp implements IImporter {
	...
	public LocalImporterSftp(String url, String path) {
		...
		if (server.contains(":")){
			String[] tiles = server.split(":");
			server = tiles[0];
			port = Integer.parseInt(tiles[1]);
		}
	...
	public boolean importFiles(String extension) {
		...
		try {
			return importIntoLocal(server, workingDir, username, password, path, port, extension);
		} catch (Exception e) {
			error = e.getMessage();
			e.printStackTrace();
			return false;
	...
	private boolean importIntoLocal(String server, String workingDir, String username, String password, String path,
			int port, String extension) throws IOException, JSchException, SftpException {
		...
		try {
			...
			for (ChannelSftp.LsEntry entry : filelist) {
				...
				if (!linkIsdir && !entry.getAttrs().isDir()
